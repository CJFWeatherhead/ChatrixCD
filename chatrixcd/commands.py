...
class CommandHandler:
    ...
    async def handle_reaction(
        self, room: MatrixRoom, sender: str, event_id: str, reaction_key: str
    ):
        ...
        if confirmation["sender"] != sender:
            ...
            logger.info(
                f"Ignoring reaction from {sender}, confirmation was initiated by {confirmation['sender']}"
            )
        ...
        if reaction_clean in positive_reactions or reaction_lower in positive_reactions:
            ...
            await self.bot.send_message(
                room.room_id, self.message_manager.get_random_message("cancel")
            )
...
"""
        ...
        commands = [
            ...
            [
                "run <project_id> <template_id>",
                "Run a task from template",
                "🚀",
            ],
            [
                "status [task_id]",
                "Check task status (uses last task if no ID)",
                "📊",
            ],
            ["stop <task_id>", "Stop a running task", "🛑"],
            [
                "logs [on|off|task_id]",
                "Get/tail task logs (uses last task if no ID)",
                "📋",
            ],
            ...
            [
                "sessions [list|reset|clear|info]",
...
"""
    ...
    async def _list_rooms(self, room_id: str, greeting: str):
        ...
        table_html = (
            "<table><thead><tr>"
            "<th>Room Name</th><th>Room ID</th><th>Send Status</th>"
            "</tr></thead><tbody>"
        )
        for room_info in room_list:
            ...
            table_html += (
                f"<tr{row_class}><td><strong>{room_name}</strong></td>"
                f"<td><code>{room_id_escaped}</code></td><td>{status_html}</td></tr>"
            )
        table_html += "</tbody></table>"

        html_message = (
            f"{greeting_html} Here's where I hang out! 🏠<br/><br/>"
            + "<strong>Rooms I'm In</strong><br/><br/>"
            + table_html
        )
    ...
    async def exit_bot(self, room_id: str, sender: str):
        ...
        if (
            confirmation_key in self.pending_confirmations
            and self.pending_confirmations[confirmation_key].get("action") == "exit"
        ):
            # User is confirming
            del self.pending_confirmations[confirmation_key]
    ...
    async def _clear_confirmation_timeout(
        ...
        if confirmation_key in self.pending_confirmations:
            ...
            if room_id:
                await self.bot.send_message(
                    room_id, self.message_manager.get_random_message("timeout")
                )
    ...
    async def _execute_log_on(self, room_id: str, sender: str):
        ...
        if self.last_task_id is not None and self.last_task_id in self.active_tasks:
            ...
            await self.bot.send_message(
                room_id, start_message, self.markdown_to_html(start_message)
            )

            # Start the tailing task
            asyncio.create_task(self.tail_logs(room_id, self.last_task_id, project_id))
        else:
            # No active task - inform user that tailing will start with the next task
            info_message = (
                "💡 No active tasks right now. "
                f"Logs will stream automatically when you run a task with `{self.command_prefix} run`."
            )
            await self.bot.send_message(
                room_id, info_message, self.markdown_to_html(info_message)
            )
    ...
    async def _execute_task(self, room_id: str, confirmation: Dict[str, Any]):
        ...
        start_message = self.message_manager.get_random_message(
            "task_start", task_name=template_name
        )
        html_start_message = self.markdown_to_html(start_message)
        logger.info(
            f"Executing task: project={project_id}, template={template_id} ({template_name}), sending: {start_message}"
        )
        ...
        if not task:
            ...
            if kwargs:
                ...
                err_msg += "Try running without these options or check the template settings in Semaphore."
                await self.bot.send_message(
        ...
        # If global log tailing is enabled for this room, start tailing this task
        if self.global_log_tailing_enabled.get(room_id, False):
            logger.info(
                f"Global log tailing enabled, starting log tail for task {task_id}"
            )
            self.log_tailing_sessions[room_id] = {
                "task_id": task_id,
                "project_id": project_id,
    ...
    async def monitor_task(
        ...
        logger.warning(
            "Enable either 'semaphore_poll' or 'semaphore_webhook' plugin in config.json"
        )
    ...
    async def _handle_log_tailing_on(self, room_id: str, sender: str) -> bool:
        ...
        if (
            confirmation_key in self.pending_confirmations
            and self.pending_confirmations[confirmation_key].get("action") == "log_on"
        ):
            # User is confirming
            del self.pending_confirmations[confirmation_key]
            if confirmation_key in self.confirmation_message_ids:
    ...
    async def _handle_log_tailing_off(self, room_id: str, sender: str) -> bool:
        ...
        if (
            confirmation_key in self.pending_confirmations
            and self.pending_confirmations[confirmation_key].get("action") == "log_off"
        ):
            # User is confirming
            del self.pending_confirmations[confirmation_key]
            if confirmation_key in self.confirmation_message_ids:
            ...
            logger.info(
                "Log tailing 'off' requested by %s, requesting confirmation",
                sender,
            )
            event_id = await self.bot.send_message(room_id, message, html_message)
    ...
    async def _retrieve_task_logs(self, room_id: str, args: list, sender: str | None):
        ...
        if not args:
            if self.last_task_id is None:
                logger.info("Log request with no args and no last task")
                await self.bot.send_message(
                    room_id,
                    f"{greeting} No task ID provided and no previous task found.\n"
            ...
            task_id = self.last_task_id
            project_id = self.last_project_id
            logger.info("Using last task ID for logs: %s", task_id)
        else:
            try:
                task_id = int(args[0])
            except ValueError:
            ...
            if task_id in self.active_tasks:
                ...
                try:
                    ...
                    await self.bot.send_message(
                        room_id, f"{greeting} Could not retrieve task info ❌"
                    )
                    return
    ...
    async def tail_logs(self, room_id: str, task_id: int, project_id: int):
        ...
        while room_id in self.log_tailing_sessions:
            ...
            if self.log_tailing_sessions[room_id]["task_id"] != task_id:
                logger.info(
                    f"Log tailing for task {task_id} stopped - different task is being tailed"
                )
                break
            ...
            try:
                ...
                if status in ("success", "error", "stopped"):
                    logger.info(
                        f"Task {task_id} finished with status {status}, stopping log tailing"
                    )
                ...
                logger.error(
                    f"Error tailing logs for task {task_id}: {e}",
                    exc_info=True,
                )
    ...
    async def _send_log_chunk(
        self, room_id: str, task_id: int, log_chunk: str, final: bool = False
    ):
    ...
    def _ansi_to_html(self, text: str) -> str:
        r"""Convert ANSI color codes to Matrix-compatible HTML.
    ...
    def _gather_bot_system_info(self) -> tuple:
        ...
        if "memory" in status:
            mem = status["memory"]
            lines.append(
                f"• **Memory Usage:** {mem['percent']:.1f}% ({mem['used']} MB / {mem['total']} MB)"
            )
    ...
    def _gather_matrix_info(self) -> tuple:
        ...
        if "matrix_homeserver" in status:
            lines.append("• **Homeserver:** " + str(status["matrix_homeserver"]))
            info_dict["homeserver"] = status["matrix_homeserver"]

        if "matrix_user_id" in status:
            lines.append("• **User ID:** " + str(status["matrix_user_id"]))
            info_dict["user_id"] = status["matrix_user_id"]

        if "matrix_device_id" in status:
            lines.append("• **Device ID:** " + str(status["matrix_device_id"]))
        ...
        # Encryption status
        if encrypted:
            lines.append("• **E2E Encryption:** Enabled 🔒")
        else:
            lines.append("• **E2E Encryption:** Disabled")
    ...
    def _format_plugin_line(self, status: Dict[str, Any], enabled: bool) -> str:
        """Format a plugin status line for plain text display.

        Args:
            status: Plugin status dictionary
        ...
        if enabled:
            extra_info = self._get_plugin_extra_info(status)
            if extra_info:
                line += f"\n  {extra_info}"

        return line

    def _get_plugin_extra_info(self, status: Dict[str, Any]) -> str:
        """Get plugin-specific extra information for display.

        Args:
            status: Plugin status dictionary

        Returns:
            Extra info string or empty string
        """
        name = status.get("name", "")
        plugin_type = status.get("type", "generic")
        info_parts = []

        # Webhook plugin specific info
        if name == "Semaphore Webhook" or "webhook" in name.lower():
            if "gotify_url" in status:
                info_parts.append(f"Gotify: {status['gotify_url']}")
            if "websocket_connected" in status:
                ws_status = (
                    "Connected" if status["websocket_connected"] else "Disconnected"
                )
                ws_icon = "🟢" if status["websocket_connected"] else "🔴"
                info_parts.append(f"WebSocket: {ws_status} {ws_icon}")
            if "monitored_tasks" in status:
                info_parts.append(f"Monitoring: {status['monitored_tasks']} tasks")

        # Polling plugin specific info
        elif plugin_type == "task_monitor":
            if "active_tasks" in status:
                info_parts.append(f"Monitoring: {status['active_tasks']} tasks")
            elif "monitored_tasks" in status:
                info_parts.append(f"Monitoring: {status['monitored_tasks']} tasks")
            if "poll_interval" in status:
                info_parts.append(f"Poll: {status['poll_interval']}s")

        # Aliases plugin specific info
        elif "aliases" in name.lower():
            if "aliases_count" in status:
                info_parts.append(f"Aliases: {status['aliases_count']}")
            if "auto_reload" in status and status["auto_reload"]:
                info_parts.append("Auto-reload: On")

        # OIDC plugin specific info
        elif "oidc" in name.lower() or "sso" in name.lower():
            if "callback_registered" in status:
                cb_status = "Ready" if status["callback_registered"] else "Pending"
                info_parts.append(f"Status: {cb_status}")

        return " | ".join(info_parts)

    def _prepare_plugin_info(self, status: Dict[str, Any]) -> Dict[str, Any]:
        """Prepare plugin info for HTML table.
    ...
    def _build_plugin_row(self, info: Dict[str, Any], enabled: bool) -> str:
        """Build a single plugin table row.

        Args:
            info: Plugin info dictionary
            enabled: Whether plugin is enabled
        ...
        # Add extra status info if available and enabled
        if enabled:
            status_dict = info.get("status", {})
            extra_info = []

            # Webhook plugin specific details
            plugin_name = info.get("name", "")
            if "webhook" in plugin_name.lower():
                if "gotify_url" in status_dict:
                    gotify_url = html.escape(str(status_dict["gotify_url"]))
                    extra_info.append(f"Gotify: {gotify_url}")
                if "websocket_connected" in status_dict:
                    ws_connected = status_dict["websocket_connected"]
                    ws_icon = "🟢" if ws_connected else "🔴"
                    ws_text = "Connected" if ws_connected else "Disconnected"
                    extra_info.append(f"WebSocket: {ws_icon} {ws_text}")
                if "monitored_tasks" in status_dict:
                    extra_info.append(f"Tasks: {status_dict['monitored_tasks']}")

            # Generic task monitor details
            elif plugin_type == "task_monitor":
                if "active_tasks" in status_dict:
                    extra_info.append(f"Tasks: {status_dict['active_tasks']}")
                elif "monitored_tasks" in status_dict:
                    extra_info.append(f"Tasks: {status_dict['monitored_tasks']}")
                if "poll_interval" in status_dict:
                    extra_info.append(f"Poll: {status_dict['poll_interval']}s")

            # Aliases plugin specific details
            elif "aliases" in plugin_name.lower():
                if "aliases_count" in status_dict:
                    extra_info.append(f"Aliases: {status_dict['aliases_count']}")
                if "auto_reload" in status_dict and status_dict["auto_reload"]:
                    extra_info.append("Auto-reload: On")

            # OIDC plugin specific details
            elif "oidc" in plugin_name.lower() or "sso" in plugin_name.lower():
                if "callback_registered" in status_dict:
                    cb_status = (
                        "Ready" if status_dict["callback_registered"] else "Pending"
                    )
                    extra_info.append(f"Status: {cb_status}")

            if extra_info:
                # Use font tag with size attribute (Matrix-compliant)
                status_html += f"<br/><font size='2'>{', '.join(extra_info)}</font>"

        return (
            f"<tr><td><strong>{name}</strong></td><td>{description}</td>"
            f"<td>{version}</td><td>{plugin_type}</td><td>{status_html}</td></tr>"
        )

    def _build_info_html_tables(
        self,
        bot_info: dict,
        matrix_info: dict,
        matrix_connected: bool,
        matrix_encrypted: bool,
        semaphore_info: dict,
    ) -> tuple:
        ...
        if "memory" in bot_info:
            ...
            if isinstance(mem, dict):
                bot_rows.append(
                    [
                        "Memory Usage",
                        f"{mem['percent']:.1f}% ({mem['used']} MB / {mem['total']} MB)",
                    ]
                )
            else:
                ...
                bot_rows.append(
                    [
                        "Memory Usage",
                        f"{mem.percent:.1f}% ({mem.used // (1024**2)} MB / {mem.total // (1024**2)} MB)",
                    ]
                )
        ...
        if matrix_info:
            for key in ["homeserver", "user_id", "device_id"]:
                if key in matrix_info:
                    ...
                    matrix_table_html += (
                        f"<tr><td><strong>{display_key}</strong></td>"
                        f"<td>{html.escape(matrix_info[key])}</td></tr>"
                    )
            ...
            if matrix_connected:
                matrix_table_html += (
                    ...
                    f"<td>{self._color_success('Connected ✅')}</td></tr>"
                )
            else:
                matrix_table_html += (
                    f"<tr><td><strong>Status</strong></td>"
                    f"<td>{self._color_error('Disconnected ❌')}</td></tr>"
                )
    ...
    async def get_semaphore_info(self, room_id: str, sender: str | None):
        ...
        html_message = (
            f"{greeting_html} Here's the technical stuff! ℹ️<br/><br/>"
    ...
    async def list_command_aliases(self, room_id: str, sender: str | None):
        ...
        html_message = (
            f"{greeting_html} Here are your command shortcuts! 🔖<br/><br/>"
            + "<strong>Command Aliases</strong><br/><br/>"
            + table_html
        )
...
"""
    ...
    async def _start_device_verification(
    ...
    async def _auto_verify_pending(self, room_id: str, sender: str):
        ...
        verified_count = 0
        for verification in pending:
            success = await self.bot.verification_manager.auto_verify_pending(
    ...
    async def _reset_session(
        ...
        try:
            # Find the device
            all_devices = (
                await self.bot.verification_manager.get_verified_devices()
    ...
    async def handle_aliases_command(self, room_id: str, args: List[str], sender: str):
        ...
        if subcommand == "add" and len(args) >= 3:
            ...
            if alias_plugin.remove_alias(alias):
                await self.bot.send_message(room_id, f"✅ Alias removed: **{alias}**")
            else:
                await self.bot.send_message(room_id, f"❌ Alias '{alias}' not found.")
        else:
            await self.bot.send_message(
                room_id,
                "Usage:\n"
