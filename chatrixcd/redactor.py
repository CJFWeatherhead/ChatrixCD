...
class SensitiveInfoRedactor:
    ...
    def _compile_patterns(self):
        ...
        self.device_id_pattern = re.compile(
            r"\bdevice[_\s]?id[\'\"=:\s]+[\'\"]*([A-Z0-9]{6,})", re.IGNORECASE
        )
        ...
        self.transaction_id_pattern = re.compile(
            r"\b(t_[a-zA-Z0-9]{10,}|[a-zA-Z0-9]{20,}_[a-zA-Z0-9]{8,})\b"
        )
        ...
        self.fingerprint_pattern = re.compile(
            r"\b[fF]ingerprint[\'\":\s]*[\'\"]*([0-9a-fA-F]{64})[\'\"]*",
            re.IGNORECASE,
        )
    ...
    def redact(self, message: str) -> str:
        ...
        message = self.device_id_pattern.sub(
            lambda m: m.group(0)[: m.group(0).index(m.group(1))]
            + self._wrap_redacted("[DEVICE_ID_REDACTED]"),
            message,
        )
        ...
        message = self.transaction_id_pattern.sub(
            self._wrap_redacted("[TRANSACTION_ID_REDACTED]"), message
        )
        ...
        message = self.base64_data_pattern.sub(
            self._wrap_redacted("[BASE64_DATA_REDACTED]"), message
        )
        ...
        message = self.ipv4_pattern.sub(
            lambda m: m.group(1) + self._wrap_redacted("xxx.xxx"), message
        )
