...
class ChatrixBot:
    ...
    def __init__(self, config: Config, mode: str = "tui"):
        ...
        self.command_handler = CommandHandler(
            bot=self, config=config.get_bot_config(), semaphore=self.semaphore
        )
        ...
        try:
            ...
            if self.client.should_upload_keys:
                ...
                if hasattr(response, "one_time_key_counts"):
                    logger.info(
                        "Uploaded keys. One-time key counts: %s",
                        getattr(response, "one_time_key_counts", None),
                    )
                else:
            ...
            logger.error("Encryption setup error: %s", e)
            return False
        ...
        try:
            ...
            if all(key in session_data for key in required_keys):
                ...
                if (
                    ...
                    and session_data["homeserver"] == self.homeserver
                ):
    ...
    async def _build_oidc_redirect_url(self, identity_providers: list) -> tuple:
        ...
        if identity_providers and len(identity_providers) == 1:
            ...
            logger.info(
                f"Using identity provider: {identity_providers[0].get('name', provider_id)}"
            )
        else:
    ...
    async def _get_oidc_token_from_user(
        self,
        ...
        if token_callback:
            ...
            try:
                login_token = await token_callback(
    ...
    async def _auto_verify_sender_devices(self, sender: str):
        ...
        if (
            not self.client.olm
            or not hasattr(self.client, "device_store")
            or not self.client.device_store
        ):
            return
    ...
    async def reaction_callback(self, room: MatrixRoom, event: Event):
        ...
        await self.command_handler.handle_reaction(
            room, event.sender, reacted_event_id, reaction_key
        )
    ...
    async def send_startup_message(self):
        ...
        startup_message = bot_config.get(
            "startup_message",
    ...
    async def send_shutdown_message(self):
        ...
        greetings_enabled = bot_config.get("greetings_enabled", True)
        greeting_rooms = bot_config.get("greeting_rooms", [])
        shutdown_message = bot_config.get(
            "shutdown_message",
            ...
            try:
                ...
                if "not verified or blacklisted" in error_msg:
                    ...
                    if retry_count < max_retries:
                        logger.info(
                            f"Retrying sync loop in {retry_delay} seconds... (attempt {retry_count + 1}/{max_retries})"
                        )
