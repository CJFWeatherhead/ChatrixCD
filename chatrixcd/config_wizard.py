...
def parse_config_example() -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Warning: Could not parse config.json.example: {e}", file=sys.stderr)
...
def run_console_config_wizard(existing_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    config['matrix']['device_id'] = get_console_input(
        "Device ID",
        config.get('matrix', {}).get('device_id', 'CHATRIXCD')
    )
    ...
    if auth_type == 'password':
        config['matrix']['password'] = get_console_input(
            "Bot password",
            config.get('matrix', {}).get('password', '')
        )
    else:  # oidc
    ...
    config['semaphore']['url'] = get_console_input(
        "Semaphore UI URL",
        config.get('semaphore', {}).get('url', '')
    )
    
    config['semaphore']['api_token'] = get_console_input(
        "Semaphore API token",
        config.get('semaphore', {}).get('api_token', '')
    )
    ...
    if get_yes_no("Configure SSL/TLS settings?", False):
        ...
        config['semaphore']['ssl_client_cert'] = config.get('semaphore', {}).get('ssl_client_cert', '')
        config['semaphore']['ssl_client_key'] = config.get('semaphore', {}).get('ssl_client_key', '')
    ...
    config['bot']['command_prefix'] = get_console_input(
        "Command prefix",
        config.get('bot', {}).get('command_prefix', '!cd')
    )
    ...
    config['bot']['log_file'] = get_console_input(
        "Log file path",
        config.get('bot', {}).get('log_file', 'chatrixcd.log')
    )
    
    # Messages
    config['bot']['startup_message'] = get_console_input(
        "Startup message",
        config.get('bot', {}).get('startup_message', '🤖 ChatrixCD bot is now online and ready to help with CI/CD tasks!')
    )
    
    config['bot']['shutdown_message'] = get_console_input(
        "Shutdown message",
        config.get('bot', {}).get('shutdown_message', '👋 ChatrixCD bot is shutting down. See you later!')
    )
