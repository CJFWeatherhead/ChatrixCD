    ...
    def test_init_flag_creates_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', new_config, '--init'],
    ...
    def test_admin_and_room_flags_override(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', 
                 '-c', temp_config,
                 '-a', '@cli_admin1:example.com',
                 '-a', '@cli_admin2:example.com',
                 '-r', '!cli_room1:example.com',
                 '-r', '!cli_room2:example.com',
                 '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
    ...
    def test_first_time_user_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                ...
                result.returncode == 0  # Proceeds with defaults
            )
