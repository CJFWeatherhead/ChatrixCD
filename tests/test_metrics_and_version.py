...
class TestVersionDetection(unittest.TestCase):
    ...
    def test_full_version_with_commit(self):
        ...
        if os.path.exists(git_dir):
            self.assertIn('-c', __version_full__, "Should include commit ID when in git repo")
            # Check commit ID format (should be hex)
            commit_part = __version_full__.split('-c')[1]
            self.assertTrue(all(c in '0123456789abcdef' for c in commit_part),
...
class TestMetricsStructure(unittest.TestCase):
    ...
    def test_metrics_keys(self):
        ...
        expected_keys = {'messages_sent', 'requests_received', 'errors', 'emojis_used'}
...
class TestEmojiCounting(unittest.TestCase):
    ...
    def test_emoji_pattern(self):
        ...
        test_cases = [
            ...
            ("No emojis here", 0),
            ("Mixed 🚀 text 🌟 with 🎯 emoji", 3),  # Separated emojis count separately
