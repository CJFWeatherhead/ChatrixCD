...
@unittest.skipUnless(os.getenv('INTEGRATION_CONFIG'), "INTEGRATION_CONFIG environment variable not set - integration tests require live Matrix server")
...
class ChatrixCDIntegrationTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        for i, bot_config in enumerate(self.all_bots):
            print(f"DEBUG: Bot {i}: user_id={bot_config.get('bot_user_id')}, has_access_token={'access_token' in bot_config}")
        
        # Find a bot with access token that is NOT the current bot being tested
        tester_bot = None
        for bot_config in self.all_bots:
            if (bot_config.get('bot_user_id') != self.matrix_config.get('bot_user_id') and
                'access_token' in bot_config and bot_config['access_token']):
        ...
        self.authenticated = False
        try:
            if ('access_token' in tester_bot and
                'device_id' in tester_bot):
            ...
            try:
                ...
                if hasattr(self.client, 'device_store') and self.client.device_store:
                    ...
                    if user_devices and target_device_id in user_devices:
                        ...
                        if getattr(device, 'verified', False):
                            print(f"DEBUG: Device {target_device_id} for {target_user_id} is already verified")
                            continue
                
                # Start verification process
                print(f"DEBUG: Starting verification with {target_user_id} device {target_device_id}")
                    ...
                    for transaction_id, verification in self.client.key_verifications.items():
                        if Sas and isinstance(verification, Sas):
                            ...
                            try:
                                ...
                                print(f"DEBUG: Failed to confirm verification {transaction_id}: {e}")
                ...
                if hasattr(self.client, 'device_store') and self.client.device_store:
                    ...
                    if user_devices and target_device_id in user_devices:
                        device = user_devices[target_device_id]
                        if getattr(device, 'verified', False):
                            print(f"DEBUG: Successfully verified device {target_device_id} for {target_user_id}")
                        else:
                            print(f"DEBUG: Device {target_device_id} for {target_user_id} is not verified")
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            if self.received_messages:
                ...
                for i, event in enumerate(self.received_messages):
                    ...
                    print(f"DEBUG: Message {i}: sender={event.sender}, body='{body}' (timestamp: {event.server_timestamp})")
            ...
            for event in self.received_messages:
                if event.sender == self.matrix_config['bot_user_id']:
                    ...
                    if decrypted_body:
                        print(f"DEBUG: Found decrypted response from bot {event.sender}: '{decrypted_body}'")
                        ...
                        print(f"DEBUG: Found encrypted response from bot {event.sender} (cannot decrypt yet)")
    ...
    async def test_bot_config_valid(self):
        ...
        self.assertIn('bot_user_id', bot_config, "Matrix config should contain bot_user_id")
        self.assertIn('homeserver', self.matrix_config, "Matrix config should contain homeserver")
        self.assertIn('test_room', self.config, "Config should contain test_room")
        
        # Verify homeserver URL format
        self.assertTrue(bot_config['homeserver'].startswith('https://'),
                       "Homeserver should use HTTPS")

        # Verify bot user ID format
        self.assertTrue(bot_config['bot_user_id'].startswith('@'),
                       "Bot user ID should start with @")
        self.assertIn(':', bot_config['bot_user_id'],
