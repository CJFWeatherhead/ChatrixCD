...
class TestMainEntryPointE2E(unittest.TestCase):
    ...
    def test_missing_config_validation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', nonexistent_config, '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
            ...
            self.assertTrue(
                ...
                'configuration' in output.lower() or
                result.returncode == 0,  # May proceed with defaults
    ...
    def test_invalid_config_validation(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', temp_config, '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                timeout=5
    ...
    def test_show_config_flag(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', temp_config, '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
    ...
    def test_verbose_flags(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', temp_config, '-v', '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
    ...
    def test_log_only_mode_flag(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', temp_config, '-L', '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
    ...
    def test_color_flag(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', temp_config, '-C', '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
    ...
    def test_redact_flag(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', '-c', temp_config, '-R', '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
    ...
    def test_combined_flags(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', 
                 '-c', temp_config, '-vv', '-C', '-R', '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
    ...
    def test_admin_users_flag(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', 
                 '-c', temp_config, '-a', '@admin1:example.com', 
                 '-a', '@admin2:example.com', '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
    ...
    def test_allowed_rooms_flag(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, '-m', 'chatrixcd.main', 
                 '-c', temp_config, '-r', '!room1:example.com', 
                 '-r', '!room2:example.com', '-s', '-N'],
                cwd=str(self.project_root),
                capture_output=True,
                text=True,
                stdin=subprocess.DEVNULL,
        ...
        try:
            ...
            if os.name != 'nt':
                ...
                self.assertTrue(
                    ...
                    'access' in output.lower() or 
                    'denied' in output.lower() or
