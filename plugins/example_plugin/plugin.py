...
class ExamplePlugin(Plugin):
    ...
    async def initialize(self) -> bool:
        ...
        """
        self.logger.info(f"Initializing Example Plugin v{self.metadata.version}")
    ...
    async def send_example_notification(self, room_id: str):
        ...
        try:
            # Plain text message
            plain_text = f"Hello from Example Plugin! (message #{self.message_count})"
            
            # HTML formatted message
            html_text = f"<b>Hello from Example Plugin!</b> (message <code>#{self.message_count}</code>)"
            
            # Send message using bot
            event_id = await self.bot.send_message(room_id, plain_text, html_text)
    ...
    async def check_semaphore_connection(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                if info:
                    self.logger.info(f"Semaphore version: {info.get('version', 'unknown')}")
            else:
