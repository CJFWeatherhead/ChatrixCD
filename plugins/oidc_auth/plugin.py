...
class OIDCAuthPlugin(Plugin):
    ...
    async def cleanup(self):
        ...
        if (
            hasattr(self.bot_instance, "oidc_plugin")
            and self.bot_instance.oidc_plugin == self
        ):
    ...
    async def login_oidc(self, bot) -> bool:
        ...
        try:
            ...
            sso_redirect_url, redirect_url = await self._build_redirect_url(
                bot, identity_providers
            )
            ...
            response = await bot.client.login(
                token=login_token, device_name=bot.device_name
            )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{bot.homeserver}/_matrix/client/v3/login"
                ) as resp:
                    if resp.status == 200:
    ...
    async def _get_token_from_user(
        self,
        ...
        if self.token_callback:
            ...
            try:
                login_token = await self.token_callback(
                    sso_redirect_url, redirect_url, identity_providers
                )
                if not login_token:
