...
class SemaphorePollPlugin(TaskMonitorPlugin):
    ...
    async def initialize(self) -> bool:
        ...
        self.logger.info(
            f"Poll interval: {self.poll_interval}s, Notification interval: {self.notification_interval}s"
        )
        return True
        ...
        for task_id, task in self.monitoring_tasks.items():
            if not task.done():
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Monitoring task {task_id} cancelled during stop()"
                    )
    ...
    async def monitor_task(
        ...
        self.logger.info(
            f"Started monitoring task {task_id} (poll interval: {self.poll_interval}s)"
        )
        ...
        try:
            while task_id in self.active_tasks and self.monitoring_active:
                ...
                if status != last_status:
                    self.logger.info(
                        f"Task {task_id} status changed: {last_status} -> {status}"
                    )
                    ...
                    if status == "running":
                        ...
                        await self._maybe_start_log_tailing(
                            task_id, project_id, room_id, task_display
                        )
                        ...
                        await self._send_completion_notification(
                            ...
                            task_display,
                            "❌",
                            f"failed or was stopped (status: {status})",
                        )
                        await self._cleanup_task(task_id)
                        break
            ...
            try:
                ...
                self.logger.debug(
                    f"Failed to send error notification for task {task_id}",
                    exc_info=True,
                )
        finally:
            await self._cleanup_task(task_id)
    ...
    async def _send_completion_notification(
        ...
        if hasattr(self.bot, "command_handler"):
            sender_name = (
                self.bot.command_handler._get_display_name(sender)
                if sender
                else "there"
            )
        else:
    ...
    async def _maybe_start_log_tailing(
        self, task_id: int, project_id: int, room_id: str, task_display: str
    ):
